...
echo " --------- Run BackendApi ------------ "
docker run -e ASPNETCORE_URLS=http://+:${BACKEND_API_PORT} -e NATS_HOST=${NATS_HOST} -e NATS_PORT=${NATS_PORT} -e REDIS_HOST=${REDIS_HOST} -e REDIS_PORT=${REDIS_PORT} --rm -d -p ${BACKEND_API_PORT}:${BACKEND_API_PORT} --name backend-api --link=nats --link=redis ds-3/backend-api

echo " ---------  Run Frontend   ------------ "
docker run -e ASPNETCORE_URLS=http://+:${FRONTEND_PORT} -e BACKEND_API_HOST=$BACKEND_API_HOST -e BACKEND_API_PORT=${BACKEND_API_PORT} --rm -d -p ${FRONTEND_PORT}:${FRONTEND_PORT} --name frontend --link=backend-api ds-3/frontend
